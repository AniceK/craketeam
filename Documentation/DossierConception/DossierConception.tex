...
\begin{description}
    ...
    \item [Scores] : les scores sont des entiers, dépendant du temps réalisé pour finir la Grille. La donnée à sauvegardée "Scores" est un tableau à trois colonnes, comprenant le score lui même, le nom de la grille sur lequel il a été réalisé, et le nom du profil l'ayant réalisé. Il est accessible depuis le menu principal. Ce tableau est stocké sur un fichier propre.
    ...
    \item [Parties Sauvegardées] : elles correspondent à la sérialisation d'une collection d'objets de la classe Partie, contenant chacune un certin nombre de paramètres et d'informations, comme la grille utilisée, l'utilisateur, le temps écoulé, etc. L'utilisateur peut ainsi quitter une partie, fermer le programme, et reprendre cette partie plus tard. La collection de Parties est sauvegardée sur un fichier propre.
\end{description}

\subsection{Méchanismes de Sauvegarde des Données}
Cette partie détaille l'ensemble des méchanismes de sauvegarde des données précédemment décrites, c'est à dire quand et comment ces données seront sauvegardées.
...
\begin{itemize}
    ...
    \item Les Scores sont sauvegardés à chaque fin de parties. Ils sont stockés dans un tableau, comme indiqué ci-dessus. À chauqe fois qu'un nouveau score est réalisé, il est ajouté au tableau, et le tableau est sérialisé et sauvegardé dans un fichier propre via le module YAML. Le tableau est chargé depuis ce fichier à chaque lancement du jeu via le module YAML.
