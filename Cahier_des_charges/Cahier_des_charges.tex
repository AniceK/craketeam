...
\newcommand{\hmwkDueDate}{Vendredi 22 février 2013} % Date
...
Ce projet s'inscrit dans le cadre du second semestre de la 3ème année de la Licence SPI (Sciences Pour l'Ingénieur) enseignée à l'Université du Mans. Il a pour but de faire développer un logiciel à une équipe d'étudiants (ici, 7 étudiants) en mettant en œuvre les concepts de Génie Logiciel vus en cours au premier semestre.\\
En effet, des professeurs de l'Université jouant le rôle de client, commandent un logiciel de jeu d'aventure simple. Tout le monde devra pouvoir utiliser ce logiciel, car il doit au final être "commercialisable".


\subsection{Objectifs}

Le logiciel à concevoir est un jeu d'aventure simple, en tour par tour, dans lequel un personnage se déplace et interagit dans un monde fini.\\
Le but général pour le joueur est de se déplacer le plus longtemps possible. Il peut y trouver des objets ainsi que des Personnages Non Joueurs (PNJ) ennemis ou apportant de l'aide. Le jeu ne propose pas de scénario ou de quête mais il se termine quand le joueur "meurt". L’utilisateur doit donc jouer le plus longtemps possible afin d'améliorer son score.


\subsection{Utilisateur cible}

Ce logiciel ne prévoit qu'une seule catégorie d'utilisateur~: le joueur. Le jeu ne permet qu'à un seul joueur de jouer simultanément.


\subsection{État de l'art}

Le projet reprend quelques idées de jeu de rôle en tour par tour telles que~:
\begin{itemize}
   \item La présence d'expérience et de niveaux, permettant au joueur de faire évoluer son personnage~;
   \item L'utilisation d'objets octroie des bonus lors des combats, ce qui en facilite l'issue et amène le joueur à développer une stratégie de consommation d'objets~;
   \item Un classement permettant aux joueurs de comparer leur progression.
\end{itemize}
